...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qk.masked_fill(torch.tril(torch.ones(T, T, device=dv)) == 0, float('-inf'))
...
class GPT(nn.Module):
    def __init__(self, vocab_size, dim, n_heads, n_layers, seq_len):
        ...
        self.blocks = nn.ModuleList([TransformerBlock(dim, n_heads) for i in range(n_layers)])
        self.ln = nn.LayerNorm(dim, device=dv)
        self.proj = nn.Linear(dim, vocab_size, device=dv)

    def cross_entropy(self, logits, label, num_classes):
        target = F.one_hot(torch.tensor(label), num_classes).to(dv)
